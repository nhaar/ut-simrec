...
// 4th step is to define the code places
...
Dictionary<string, string> getCodePlace (string codeName, int type) {
    Dictionary<string, string> codePlace = new Dictionary<string, string> { { "CodeName", codeName }, { "Type", type.ToString() } };
    return codePlace;
}

Dictionary<string, string> getCodePlace (string codeName, int type, string location) {
    Dictionary<string, string> codePlace = getCodePlace(codeName, type);
    codePlace["Location"] = location;
    return codePlace;
}

Dictionary<string, Dictionary<string, string>> eventInfo = new Dictionary<string, Dictionary<string, string>> {
    {
        "PickName",
        getCodePlace(naming, (int)CodeInput.PlaceInIf, "naming = 4")
    },
    {
        "Blcon",
        getCodePlace(blcon, (int)CodeInput.Append)
    },
    {
        "EnterBattle",
        getCodePlace(blconAlarm, (int)CodeInput.Append)
    }
};
...
foreach (string eventName in events.Keys) {
    ...
    Dictionary<string, string> codePlace = eventInfo[eventName];
    switch (Int32.Parse(codePlace["Type"])) {
        case (int)CodeInput.Append:
            append(codePlace["CodeName"], eventCode);
            break;
        case (int)CodeInput.Place:
            place(codePlace["CodeName"], codePlace["Location"], eventCode);
            break;
        case (int)CodeInput.PlaceInIf:
            replace(codePlace["CodeName"], codePlace["Location"], @$"
            {{
                {codePlace["Location"]};
                {eventCode}
            }}
            ");
            break;
        case (int)CodeInput.Replace:
            replace(codePlace["CodeName"], codePlace["Location"], eventCode);
            break;
    }
}
